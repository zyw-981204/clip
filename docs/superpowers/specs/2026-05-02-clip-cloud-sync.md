...
| key | value | 谁写 | 何时 |
|---|---|---|---|
| `schema_version` | `"3"` (current) | client 在 ensureSchema 末尾 INSERT OR IGNORE | 首启 |
| `kdf_iters` | `"200000"` | 同上 | 首启 |
| `kdf_salt_b64` | `"<16 random bytes base64>"` | 同上；INSERT OR IGNORE 保证 race 时只有一份 salt 留下 | 首启 |

**Schema 版本守门**：每次 SyncEngine 启动 `ensureSchema()` 之后 SELECT `schema_version`：
- 远端 = 本地 → 正常
- 远端 < 本地 → 本地新；写本地版号覆盖（client 内部跑迁移逻辑，但 v3 没迁移要跑）
- 远端 > 本地 → **本地老**；不能保证字段语义一致；UI 弹"请升级 Clip"对话框 + 暂停 sync（不冒险破坏数据）

v3 是初始版，`schema_version='3'`。v3.x 加列 / 改语义时新写一版 + 跑老→新 ALTER；这里只确立"老 client 不会瞎跑"的护栏。

`ensureSchema()` 在 SyncEngine 每次启动时跑一遍。所有语句都是 `IF NOT EXISTS` / `INSERT OR IGNORE`，幂等；旧 D1 数据库不会被破坏；多 client 并发首次启用也不会冲突。
...
push 前 SyncEngine 查 D1 `SELECT id, deleted FROM clips WHERE hmac = ? LIMIT 1`（**`deleted=1` 的也算命中**）；命中 → 复用其 `cloud_id`，走 UPDATE。这同时处理两件事：
- 普通跨设备 dedup（A B 同时 capture 同 hash → 第二个 push 改成 UPDATE）
- "排除后又复制"复活语义：A 排除 → tomb (`deleted=1`)；用户后悔 / B 重新 capture → push 命中老 cloud_id → UPSERT 把同行翻回 `deleted=0`，而**不是**新插一条同 hmac 的行（避免云端冗余 row）
...
用户在 Preferences > 云同步 → 输入 R2 endpoint / bucket / access key / secret +
  ...
  ↓ 按 "测试连接"
  ↓ ensureSchema() 在 D1 上跑（CREATE TABLE IF NOT EXISTS clips/devices/config + 必要索引）
  ↓ 全部走 idempotent 写法（CREATE INDEX IF NOT EXISTS 同样适用）
  ↓
  ↓ 自我 + 远端 schema_version 守门：
  ↓   remote_v = getConfig("schema_version") ?? "3"   // 缺 = 没人设过，默认 v3 等价
  ↓   if Int(remote_v) > Int(local_v) → 抛 SyncError.remoteSchemaNewer
  ↓                                       → UI 弹"请升级 Clip" + 暂停 sync
  ↓
  ↓ INSERT OR IGNORE INTO config 三条：
  ↓   ('schema_version', '3')
  ↓   ('kdf_iters', '200000')
  ↓   ('kdf_salt_b64', <16 random bytes base64>)   // 仅当本地 generate；见下
  ↓
  ↓ SELECT value FROM config WHERE key='kdf_salt_b64' → salt（权威值，可能我赢可能对方先到）
  ↓ SELECT value FROM config WHERE key='kdf_iters'    → iters
  ↓
  ↓ 派生 master_key = PBKDF2(密码, salt, iters)
  ↓ master_key → Keychain (kSecAttrSynchronizable=false)
  ↓
  ↓ firstDevice 判定：salt INSERT 自己赢了（rows_written > 0）→ 视为第一台 → backfill
  ↓ 否则（INSERT OR IGNORE 没动）→ 视为加入现有 profile → 不 backfill
  ↓
...
pullTask:
  // 复合游标 (updated_at, id) — 防止 D1 unixepoch() 秒精度下同秒多 push 漏拉
  cursor = sync_state.get("cloud_pull_cursor") ?? "0:"   // "<unix_sec>:<last_id_uuid>"
  loop {
    rows = try await dataSource.queryClipsChangedSince(cursor: cursor, limit: 100)
    // SQL 语义：WHERE updated_at > ?ts OR (updated_at = ?ts AND id > ?id)
    //          ORDER BY updated_at, id
    if rows.isEmpty: break
    for row in rows:
      // 0. LWW-equivalent skip：本地 cloud_updated_at 已 >= 远端 → 不重做
      if local = store.itemByCloudID(row.id), local.cloudUpdatedAt >= row.updatedAt:
        // 仍然要 advance cursor 到该行（否则下轮还会拉它）
        cursor = "\(row.updatedAt):\(row.id)"
        continue
  ...
  sync_state.set("cloud_pull_cursor", cursor)
...
| 远端 D1 `schema_version` > 本地 client | client 启动检测到 → 抛 `SyncError.remoteSchemaNewer(remote, local)` → 暂停 sync + UI 弹"另一台设备装了更新版本的 Clip，请升级本机后重试"；不强制升级也不破坏数据 |
...
| R2 流量 / ops 异常 | 同上 |
| 半完成 push（R2 PUT 成功 + D1 UPSERT 失败） | R2 留下"孤儿 blob"。下次重试：R2 PUT 同 hmac 文件名是幂等的（覆盖同字节）+ D1 UPSERT 重做。最终 D1 row + R2 blob 一致；中间态留下的孤儿无害。**v3 不主动 GC**；R2 收费 ~$0.000015/MB·月，可忽略。v3.1 可以加 R2 lifecycle rule 自动清 30 天没人引用的 `blobs/<hmac>.bin` |
